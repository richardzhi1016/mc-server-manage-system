...
@files_bp.route("/<server_name>/files/<path:path>", methods=["GET"])
def read_server_file(server_name: str, path: str):
    ...
    try:
        ...
        return jsonify({
            "content": content,
            "file": server_manager.get_file_info(abs_path, server_dir),
        }), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@files_bp.route("/<server_name>/files/<path:path>", methods=["PUT"])
...
@files_bp.route("/<server_name>/files/<path:path>", methods=["POST"])
...
@files_bp.route("/<server_name>/files/<path:path>", methods=["DELETE"])
...
def upload_server_file(server_name: str):
    ...
    is_valid, error, abs_base_path = server_manager.validate_server_path(
        server_name, path
    )
    if not is_valid:
        return jsonify({"error": error}), 403
...
def rename_server_file(server_name: str):
    ...
    is_valid, error, abs_path = server_manager.validate_server_path(
        server_name, path
    )
    if not is_valid:
        return jsonify({"error": error}), 403
...
@files_bp.route("/<server_name>/download/<path:path>", methods=["GET"])
